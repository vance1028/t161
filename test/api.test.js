...
test('订餐：A 级长者（低保+失能），正确叠加基础+身份+封顶额度返回', async () => {
  const token = await loginAs('operator', 'operator123');
  const meals = (await request(app).get('/api/meals').set('Authorization', `Bearer ${token}`)).body.data;
  // 新建一个干净的 A 级长者（种子里 e1 已有订单，会影响额度累计测试）
  const newElder = await request(app).post('/api/elders').set('Authorization', `Bearer ${token}`)
    .send({ code: 'E-T-001', name: '测试A老', gender: 'M', age: 75, phone: '13910000001', subsidyLevel: 'A', identities: 'LOW_INCOME,DISABLED' });
  assert.strictEqual(newElder.status, 201);
  const elder = newElder.body.data;
...
test('订餐：B 级长者（高龄）+ 晚餐，正确叠加基础+高龄+餐别加补', async () => {
  ...
  // 新建一个干净的 B 级高龄长者
  const newElder = await request(app).post('/api/elders').set('Authorization', `Bearer ${token}`)
    .send({ code: 'E-T-002', name: '测试B老', gender: 'F', age: 83, phone: '13910000002', subsidyLevel: 'B', identities: 'SENIOR_80' });
  assert.strictEqual(newElder.status, 201);
  const elder = newElder.body.data;
...
test('退订：实际补贴归零，明细删除，月度额度释放', async () => {
  ...
  // 新建干净长者，避免种子已有订单干扰
  const newElder = await request(app).post('/api/elders').set('Authorization', `Bearer ${token}`)
    .send({ code: 'E-T-003', name: '测试C老', gender: 'M', age: 72, phone: '13910000003', subsidyLevel: 'A', identities: 'LOW_INCOME' });
  assert.strictEqual(newElder.status, 201);
  const elder = newElder.body.data;
...
test('结算生成接口：路由不被 :id 抢占，返回 200 且 SUMMARY+三维度齐全', async () => {
  const token = await loginAs('operator', 'operator123');
  ...
  const { month, summary, sheets, detailSheetCount, discrepancyCount, totals, capRules, holidayCount } = gen.body.data;
  assert.strictEqual(month, '2026-06');

  // SUMMARY 总单
  assert.ok(summary, '应有 summary 总单');
  assert.strictEqual(summary.sheetType, 'SUMMARY');
  assert.strictEqual(summary.groupKey, 'summary');
  assert.ok(summary.orderCount > 0);
  assert.ok(summary.totalSubsidyCents > 0);

  // 三维度明细单
  const types = sheets.map((s) => s.sheetType);
  assert.ok(types.includes('SUMMARY'), '应有 SUMMARY 类型');
  ...
  assert.ok(Array.isArray(sheets) && sheets.length >= 4, `至少生成 4 张结算单（1 SUMMARY + 3 维度），实际 ${sheets.length}`);
  assert.strictEqual(detailSheetCount, sheets.length - 1, 'detailSheetCount = sheets.length - 1');
...
test('干净种子数据：结算零差异（种子订单与规则完全自洽）', async () => {
  const token = await loginAs('operator', 'operator123');
  // 不新增订单，直接用种子数据生成结算
  const gen = await request(app).post('/api/settlements/generate').set('Authorization', `Bearer ${token}`)
    .send({ month: '2026-06' });
  assert.strictEqual(gen.status, 200, JSON.stringify(gen.body));
  assert.strictEqual(gen.body.data.discrepancyCount, 0,
    `干净种子不应有差异，实际: ${JSON.stringify(gen.body.data.discrepancyByType)}`);
  assert.ok(gen.body.data.summary, '应有 SUMMARY 总单');
});

test('勾稽差异：退订未回退识别为 CANCELLED_WITH_SUBSIDY，且差异归属 SUMMARY 总单', async () => {
  const admin = await loginAs('admin', 'admin123');
  const operator = await loginAs('operator', 'operator123');
  ...
  const gen = await request(app).post('/api/settlements/generate').set('Authorization', `Bearer ${operator}`)
    .send({ month: '2026-06' });
  assert.strictEqual(gen.status, 200, JSON.stringify(gen.body));
  const { discrepancyByType, summary } = gen.body.data;
  ...
  // 差异记录挂在 SUMMARY 总单下
  const getSummary = await request(app).get(`/api/settlements/${summary.id}`).set('Authorization', `Bearer ${operator}`);
  assert.strictEqual(getSummary.status, 200);
  assert.ok(getSummary.body.data.discrepancies.length > 0,
    'SUMMARY 总单下应挂有差异记录');
  const hasIssue = getSummary.body.data.discrepancies.some((d) => d.issueType === 'CANCELLED_WITH_SUBSIDY');
  assert.ok(hasIssue, 'SUMMARY 下的差异包含 CANCELLED_WITH_SUBSIDY');

  // 维度明细单下不应有差异
  const sheets = gen.body.data.sheets;
  const byCanteen = sheets.find((s) => s.sheetType === 'BY_CANTEEN');
  if (byCanteen) {
    const getDetail = await request(app).get(`/api/settlements/${byCanteen.id}`).set('Authorization', `Bearer ${operator}`);
    assert.strictEqual(getDetail.body.data.discrepancies.length, 0,
      '维度明细结算单下不应有差异记录');
  }
});

test('结算单确认：确认 SUMMARY 总单后自动锁定对应月份', async () => {
  const admin = await loginAs('admin', 'admin123');
  const operator = await loginAs('operator', 'operator123');
  ...
  const summarySheet = gen.body.data.summary;
  const detailSheet = gen.body.data.sheets.find((s) => s.sheetType === 'BY_LEVEL');

  // 先确认维度明细单，不会自动锁定
  const confirmDetail = await request(app)
    .post(`/api/settlements/${detailSheet.id}/confirm`)
    .set('Authorization', `Bearer ${admin}`);
  assert.strictEqual(confirmDetail.status, 200);
  assert.strictEqual(confirmDetail.body.data.monthLocked, false,
    '确认维度明细单不应自动锁定月份');

  // 确认月份尚未锁定
  const lockBefore = await request(app).get('/api/settlements/locks/2026-06').set('Authorization', `Bearer ${admin}`);
  assert.strictEqual(lockBefore.body.data.locked, false, '确认 SUMMARY 前月份不应锁定');

  // 确认 SUMMARY 总单，应自动锁定
  const confirmSummary = await request(app)
    .post(`/api/settlements/${summarySheet.id}/confirm`)
    .set('Authorization', `Bearer ${admin}`);
  assert.strictEqual(confirmSummary.status, 200);
  assert.strictEqual(confirmSummary.body.data.monthLocked, true,
    '确认 SUMMARY 总单应自动锁定月份');
  assert.ok(confirmSummary.body.data.lock, '应返回 lock 对象');

  // 验证月份已锁定
  const lockAfter = await request(app).get('/api/settlements/locks/2026-06').set('Authorization', `Bearer ${admin}`);
  assert.strictEqual(lockAfter.body.data.locked, true, '确认 SUMMARY 后月份应已锁定');
  assert.ok(lockAfter.body.data.lock.remark.includes('自动锁定'),
    '锁定备注应包含"自动锁定"字样');
